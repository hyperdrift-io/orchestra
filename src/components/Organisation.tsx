...
export function Organisation({articlesAvailable=false}:{articlesAvailable?:boolean}){
  ...
  useEffect(()=>{
    try{
      engine.current=createConstellation({canvas:canvas.current!,host:surface.current!,nodes:()=>nodes.current,facets:()=>facets.current,onStep:setStep,onCycle:setCycle,onComplete:setComplete,onPlaying:setPlaying,onMotion:setReduced,onFocus:index=>{
  ...
  return <section id="organisation" data-domain={step+1} data-focused={focused!==null} onKeyDown={event=>{if(event.key==='Escape'&&focused!==null){event.preventDefault();reset();}}} aria-labelledby="organisation-title">
    ...
    <div data-constellation-view><div data-constellation-surface ref={surface}>
      <canvas ref={canvas} aria-label="Living constellation: five coloured domains connected by branching business signals. Select a sphere to explore its action and impact."/>
      {steps.map((item,i)=><button key={item.name} hidden={!available||focused!==null} ref={el=>{nodes.current[i]=el;}} data-domain-label={i+1} aria-label={`Explore ${item.name}`} aria-pressed={step===i} onClick={()=>select(i)}>{item.name}</button>)}
      {['Signal','Action','Impact','Measure'].map((label,i)=><button key={label} hidden={focused===null} ref={el=>{facets.current[i]=el;}} data-domain-facet aria-pressed={facet===i} aria-label={`Explore ${label.toLowerCase()} facet`} onClick={()=>{setFacet(i);engine.current?.facet(i);}}>{label}</button>)}
      <p>{focused!==null?`Inside ${steps[step].name} · click outside the sphere or press Escape to return`:'Follow the light. Select a sphere to see what happens inside.'}</p>
      {available===false&&<p role="status">Explore every action and impact using the stages below.</p>}
    </div>
    {focused!==null&&<aside data-domain-story aria-live="polite"><p>{steps[step].name} · {iteration.title}</p><h2>{facet===null?steps[step].title:['The signal','The action','Business impact','What we measure'][facet]}</h2><p>{facet===null?steps[step].action:[steps[step].signal,steps[step].action,steps[step].impact,steps[step].measure][facet]}</p><button onClick={reset}>← Whole organisation</button></aside>}
    </div>
    ...
    <div data-organisation-story>
      ...
      <nav aria-label="Business domains">{steps.map((item,i)=><button key={item.name} aria-pressed={step===i} onClick={()=>select(i)}>{item.name}</button>)}</nav>
