...
export function SystemGraph({scenario=businessScenarios[0]}:{scenario?:BusinessScenario}) {
  ...
  return <div data-business-graph="globe" data-focused={focused}>
    ...
    <div data-graph-surface ref={surface} onPointerEnter={e=>{if(e.pointerType==='mouse')engine.current?.hover();}} onPointerLeave={()=>engine.current?.hover(-1)}>
      <canvas ref={canvas} aria-label="Living business network. Select a numbered node to move inside and explore its business facets." />
      ...
      {['Signal','Action','Impact','Measure'].map((label,i)=><button key={label} hidden={!focused} ref={el=>{facets.current[i]=el;}} data-facet={label} aria-label={`Explore ${label.toLowerCase()} facet`} aria-pressed={facet===i} onClick={()=>{setFacet(i);setAnnouncement(`${label}: ${[scenario.steps[step].signal,scenario.steps[step].action,scenario.steps[step].impact,scenario.steps[step].measure][i]}`);}}><span>{label}</span></button>)}
      {!available&&<p role="status">The animated view is unavailable. Every action and impact is available below.</p>}
      <p data-graph-instruction>{focused?`Inside ${scenario.steps[step].name} · choose a facet`:'Hover to turn. Select a node to go inside.'}</p>
    </div>
