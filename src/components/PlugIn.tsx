...
export function PlugIn(){
  ...
  useEffect(()=>{
    ...
    const run=()=>{clearInterval(timer);if(visible&&playing&&!document.hidden&&!media.matches)timer=setInterval(()=>setActive(i=>(i+1)%5),4200);};
  ...
  return <section id="plug" ref={section} aria-labelledby="plug-title" data-step={active} data-playing={playing}>
    ...
    <figure data-organisation-view data-zoomed={zoom} tabIndex={0} aria-label="Interactive organisation graph. Scroll horizontally on smaller screens."><div><OrgGraph id="graph-lens" scenario={scenario} active={active} onSelect={select}/><OrgPulse active={active} playing={playing}/></div></figure>
