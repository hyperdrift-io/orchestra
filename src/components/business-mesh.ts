...
export interface MeshEngine { select(index:number):void; zoom(value:number):void; play(value:boolean):void; hover(index?:number):void; reset():void; dispose():void; }
interface Options { canvas:HTMLCanvasElement; host:HTMLDivElement; nodes:()=>Array<HTMLButtonElement|null>; facets:()=>Array<HTMLButtonElement|null>; onStep:(n:number)=>void; onFocus:(n:number)=>void; onMotion:(reduced:boolean)=>void; }
...
export function createBusinessMesh(o:Options):MeshEngine {
  ...
  const facetPositions=new Float32Array(24);
  const facetGeo=new THREE.BufferGeometry();facetGeo.setAttribute('position',new THREE.BufferAttribute(facetPositions,3));
  const facetMat=new THREE.LineBasicMaterial({color:gold,transparent:true,opacity:0});
  const facetLines=new THREE.LineSegments(facetGeo,facetMat);scene.add(facetLines);
  const facetPoints=Array.from({length:4},()=>new THREE.Vector3());
  const facetsMesh=new THREE.InstancedMesh(geo,signalMat,28);scene.add(facetsMesh);facetsMesh.visible=false;
  ...
  const render=(dt=0)=>{
    ...
    facetsMesh.visible=focused!==null;facetMat.opacity=focused!==null?.8:0;
    if(focused!==null){
      const source=anchors[focused],offsets=[[-.62,.48],[.62,.48],[-.62,-.48],[.62,-.48]];
      
      offsets.forEach(([x,y],i)=>{facetPoints[i].copy(source).add(new THREE.Vector3(x,y,.25));placeButton(o.facets()[i],facetPoints[i],true);facetPositions.set(source.toArray(),i*6);facetPositions.set(facetPoints[i].toArray(),i*6+3);});facetGeo.attributes.position.needsUpdate=true;
      for(let i=0;i<28;i++){const p=facetPoints[i%4];temp.copy(p).add(new THREE.Vector3(Math.sin(i*2.7+time*.5)*.2,Math.cos(i*1.7+time*.3)*.17,Math.sin(i)*.1));matrix.makeScale(.012,.012,.012);matrix.setPosition(temp);facetsMesh.setMatrixAt(i,matrix);}facetsMesh.instanceMatrix.needsUpdate=true;
    }else o.facets().forEach(button=>{if(button){button.style.opacity='0';button.style.pointerEvents='none';button.tabIndex=-1;}});
  ...
  const select=(index:number)=>{current=index;focused=index;hoverTarget=null;stageTime=0;flight=true;o.onFocus(index);render(media.matches?1:.016);schedule();};
  ...
  const up=(e:PointerEvent)=>{if(focused!==null||Math.hypot(e.clientX-startX,e.clientY-startY)>8)return;const rect=o.canvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top;const hit=projectedNodes.map(p=>({...p,d:Math.hypot(x-p.x,y-p.y)})).filter(p=>p.d<24).sort((a,b)=>a.d-b.d||b.z-a.z)[0];if(hit)select(hit.stage);};
