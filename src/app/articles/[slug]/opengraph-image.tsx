...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return new ImageResponse(
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', padding: 55, background: '#181510', color: '#f6eee1' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 18, color: '#e3a857', letterSpacing: 2 }}><div style={{ display: 'flex', alignItems: 'center', gap: 16 }}><img src={logo} width={48} height={48} alt="" /><span>THE AI-NATIVE ORGANISATION</span></div><span>{String(entry.order).padStart(2, '0')} / HYPERDRIFT</span></div>
